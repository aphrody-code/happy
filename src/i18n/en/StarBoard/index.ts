import type { NamespaceStarBoardTranslation } from '../../i18n-types'

const en_StarBoard = {
	STARBOARD: {
		STAR_SLC_CHAN: {
			MESSAGE: 'Please select a channel first!',
		},
		STAR_CHAN_SET: {
			MESSAGE: 'Starboard channel set!',
		},
		STAR_CFG_UPDATED: {
			MESSAGE: 'Starboard config updated!',
		},
		STAR_ERR_EMOJI: {
			MESSAGE: 'Please provide a correct emoji (do not use combined emojis)!',
		},
		STAR_ERR_PARAM: {
			MESSAGE: 'Please provide at least one parameter!',
		},
		STAR_ERR_GUILD: {
			MESSAGE: 'This command must be used inside a server!',
		},
		STAR_MIN_EMOJI: {
			DESCRIPTION: 'Set the minimum number of stars to be displayed',
		},
		STAR_SET_EMOJI: {
			DESCRIPTION: 'Set the emoji to use for the starboard',
		},
		STAR_SET_CHAN: {
			DESCRIPTION: 'Set the channel to use for the starboard',
		},
		STAR_DESC: {
			DESCRIPTION: 'Set the starboard channel',
		},
	},
} satisfies NamespaceStarBoardTranslation

export default en_StarBoard
