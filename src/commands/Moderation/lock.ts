...
@Injectable()
@Category('Moderation')
export default class LockCommand {
	...
	async lock(
		@SlashOption({
			name: 'channel',
			localizationSource: 'COMMANDS.LOCK.OPTIONS.CHANNEL',
			type: ApplicationCommandOptionType.Channel,
		}) targetChannel: TextChannel | undefined,
		@SlashOption({
			name: 'reason',
			localizationSource: 'COMMANDS.LOCK.OPTIONS.REASON',
		...
		log.action = ModerationAction.LOCK
		...
		simpleSuccessEmbed(
			interaction,
			localize.COMMANDS.LOCK.SUCCESS({ channel: `<#${channel.id}>` })
		)
	}
